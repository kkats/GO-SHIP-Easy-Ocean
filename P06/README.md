...
### 2. Procedure
1. Download and unzip *all* CTD files necessary to form one complete section. For example, three zip files `p06e_nc_ctd.zip`, `p06c_nc_ctd.zip`, and `p06w_nc_ctd.zip` are necessary for the 1992 section. Unzip the archive *in one directory*. At this moment, **whp_netcd** is the preferred format.
1. Start Matlab and run `read_ctd_nc`.
~~~
>> read_ctd_nc('P06/CTD', 'P06/p06raw');
~~~
...
1. Run `findJOAstations`.
~~~
>> findJOAstations('P06/JOA/P06_1992_bottle.csv', stations);
~~~
